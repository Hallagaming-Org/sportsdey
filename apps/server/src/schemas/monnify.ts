...
export const MonnifyProductSchema = z
	.object({
		...
		billers: z
			.array(
				z.object({
					code: z.string().openapi({ description: "Biller code" }),
					name: z.string().openapi({ description: "Biller name" }),
				}),
			)
		...
		metadata: z
			.object({
				...
				productType: z
					.object({
						code: z.string().openapi({ description: "Product type code" }),
						name: z.string().openapi({ description: "Product type name" }),
					})
					.openapi({ description: "Product type" }),
				durationUnit: z
					.string()
					.nullable()
					.openapi({ description: "Duration unit" }),
				productCategory: z
					.string()
					.nullable()
...
export const MonnifyProductsResponseSchema = z
	.object({
...
export const MonnifyValidationSchema = z
	.object({
...
export const MonnifyVendResponseSchema = z
	.object({
...
export const MonnifyRequerySchema = z
	.object({
		transactionReference: z.string().openapi({
			description: "Monnify transaction reference",
		}),
