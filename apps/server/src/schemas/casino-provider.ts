...
export const AuthRequestSchema = z
	.object({
		...
		session_token: z
			.string()
			.openapi({ description: "Provider session token" }),
		platform: z
			.string()
			.optional()
		...
		data: z
			.object({
				...
				balance: z.number().openapi({
					description: "Balance in provider units (1 NGN = 1000 units)",
				}),
...
export const WithdrawRequestSchema = z
	.object({
		...
		amount: z.number().openapi({
			description: "Amount in provider units (1 NGN = 1000 units)",
		}),
		currency: z
			.string()
			.openapi({ description: "Bet currency (currency code)" }),
		provider_tx_id: z
			.string()
...
export const CasinoWithdrawResponseSchema = z
	.object({
		code: z.number().openapi({ description: "Response code" }),
		data: z
			.object({
				...
				provider_tx_id: z
					.string()
					.openapi({ description: "Provider transaction ID" }),
				old_balance: z.number().openapi({
					description:
						"Balance before bet in provider units (1 NGN = 1000 units)",
				}),
				new_balance: z.number().openapi({
					description:
						"Balance after bet in provider units (1 NGN = 1000 units)",
				}),
...
export const DepositResponseSchema = z
	.object({
		code: z.number().openapi({ description: "Response code" }),
		data: z
			.object({
				...
				amount: z.number().openapi({
					description: "Win amount in provider units (1 NGN = 1000 units)",
				}),
				old_balance: z.number().openapi({
					description:
						"Balance before win in provider units (1 NGN = 1000 units)",
				}),
				new_balance: z.number().openapi({
					description:
						"Balance after win in provider units (1 NGN = 1000 units)",
				}),
			})
			.openapi({ description: "Response data" }),
	})
	...
	.object({
		...
		data: z
			.object({
				...
				old_balance: z.number().openapi({
					description:
						"Balance before rollback in provider units (1 NGN = 1000 units)",
				}),
				new_balance: z.number().openapi({
					description:
						"Balance after rollback in provider units (1 NGN = 1000 units)",
				}),
				operator_tx_id: z.string().openapi({
					description: "Operator transaction ID (game transaction ID)",
				}),
...
export const PlayerInfoRequestSchema = z
	.object({
		user_id: z.string().openapi({ description: "User ID" }),
		session_token: z.string().openapi({ description: "Game session token" }),
	})
...
export const PlayerInfoResponseSchema = z
	.object({
		code: z.number().openapi({ description: "Response code" }),
		data: z
			.object({
...
export const CasinoProviderErrorSchema = z
	.object({
